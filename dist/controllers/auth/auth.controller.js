...
import { forgetPasswordSchema, loginSchema, resetPasswordSchema, signupSchema, verifyForgetPassOTPSchema, } from "@/controllers/auth/schema.js";
import { createToken, generateOtp, hashPassword, verifyPassword, } from "@/helpers/authHelper.js";
...
export const authController = {
    ...
    async login(req, res) {
        ...
        try {
            ...
            const isPassword = await verifyPassword(req.body.password, user?.password);
            if (!isPassword) {
                res.status(HttpStatus.BAD_REQUEST).json({
    ...
    async verifyForgetPassOTP(req, res) {
